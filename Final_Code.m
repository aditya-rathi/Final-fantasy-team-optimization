...
x0 = [ones([20,4]); ones([20,4]); ones( [20,4])];
opts = optimset('Algorithm','sqp','Display','iter','MaxFunEvals',inf,'MaxIter',inf);
 [x fval x1_out x2_out x_min_out output lambda grad hessian] = runfmincon(F,x0, nonlinfc, opts);
 lambda;
 output;
 grad; 
 hessian;
 Eigen_values = eig(hessian);
 
%  x0_flat = reshape(x0,1,[]);
%  [x fval] = fmincon(F,x0_flat,[],[],[],[],zeros(1,length(x0_flat)),[], nonlinfc);

% options = optimoptions('ga','Display','iter','PopulationSize',2400, 'InitialPopulationMatrix', 2*ones(300,length(x0_flat)));
% nvars = 240
% [x Fval exitflag] = ga(F,nvars,[],[],[],[],zeros(1,length(x0_flat)),[],nonlinfc,options)
% x1_out = x(1:80);
% x2_out = x(81:160);
...
%  x1_out=reshape(x1_out,1,[]);
%  x2_out=reshape(x2_out,1,[]);
%  x_min_out=reshape(x_min_out,1,[]);
 
%  x_min_out = (x_min_out-100)./(200 - 100)
 
%  f = -sum(x_min_out.*W_flat.*x1_out,'all')- sum((1-((sum(x_min_out.*x1_out,'all'))/11))*x_min_out.*W_flat.*x2_out,'all')
...
%  runfmincon(F,x0, nonlinfc, opts);
 ...
 %% Runfmincon function
 function [x fval x1_out x2_out x_min_out output lambda grad hessian] = runfmincon(F,x0, nonlinfc, opts)
  x0_flat = reshape(x0,1,[]);
[x fval exitflag output lambda grad hessian] = fmincon(F,x0_flat,[],[],[],[],...
